...
import 'package:jeeni/pages/widgets/reset_pass_dailog.dart';
...
class AuthenticationNotifier extends StateNotifier<Student?> {
  ...
  Future<Response> loginWithIdAndPassword({
    ...
    ResetPassDailog resetPassDialog = ResetPassDailog();

    final response =
        await ref.read(networkProvider.notifier).loginWithIdAndPassword(
              userId: userId,
              password: password,
    ...
    if (response.statusCode == 200) {
      ...
      int studentId = decodedValue['id'];
      String jauthh = decodedValue['jauth'];

      if (decodedValue['passwordAutoGenerated'] == true) {
        resetPassDialog.showResetPassword(context, jauthh, studentId, password,
            () async {
          print("data  for it if button  pressed  ");
